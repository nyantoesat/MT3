...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		bool hit = IsCollision(sphere0, sphere1);

		ImGui::Begin("Window");
		ImGui::DragFloat3("CameraTranslate", &cameraTranslate.x, 0.01f);
		ImGui::DragFloat3("CameraRotate", &cameraRotate.x, 0.01f);
		ImGui::DragFloat3("Sphere[0].Center", &sphere0.center.x, 0.01f);
		ImGui::DragFloat("Sphere[0].Radius", &sphere0.radius, 0.01f);
		...
		uint32_t sphere0Color = hit ? 0xFF0000FF  : 0xFFFFFFFF ;
		uint32_t sphere1Color = 0xFFFFFFFF;
		DrawSphere(sphere0, viewProjectionMatrix, viewportMatrix, sphere0Color);
